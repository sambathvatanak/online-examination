...
class Excel_Exam{
  ...
  public function addQuestions($data){
    ...
    $query = "INSERT INTO tbl_excel_ques(quesNo,ques) VALUES('$quesNo','$ques')";
    $inserted_row = $this->db->insert($query);
    if ($inserted_row) {
      foreach ($ans as $key => $ansName) {
        if ($ansName != '') {
         if ($rightAns == $key) {
           $rquery = "INSERT INTO tbl_excel_ans(quesNo,rightAns,T_rightAns,ans,sub_ans_ID) VALUES('$quesNo','1','$rightAns','$ansName','$key')";

         }else{
          $rquery = "INSERT INTO tbl_excel_ans(quesNo,rightAns,T_rightAns,ans,sub_ans_ID) VALUES('$quesNo','0','$rightAns','$ansName','$key')";
         }
...
//alert message after updated
  ...
  public function getQues($id){
      $query = "SELECT * FROM tbl_excel_ques WHERE quesNo= '$id'";
       $result = $this->db->select($query);
       return $result;
  }

  public function getRightAns($quesNo){
    $query = "SELECT T_rightAns FROM tbl_excel_ans WHERE quesNo= '$quesNo' AND rightAns= '1'";
     $result = $this->db->select($query);
     return $result;
  }

  public function getAns($id){
      $query = "SELECT * FROM tbl_excel_ans WHERE quesNo= '$id'";
       $result = $this->db->select($query);
       return $result;
  }

  public function getQueByOrder(){
    $query = "SELECT * FROM  tbl_excel_ques ORDER BY quesNo ASC";
    $result = $this->db->select($query);
    return $result;
  }
...
$tables = array("tbl_excel_ques","tbl_excel_ans");
...
if ($deldata) {
  ...
  public function getTotalRows(){
    $query = "SELECT * FROM tbl_excel_ques";
  ...
  public function getQuesByNumber($number){
    $query = "SELECT * FROM tbl_excel_ques WHERE quesNo ='$number'";
    $getData = $this->db->select($query);
    $result = $getData->fetch_assoc();
    return $result;
  ...
  public function getAnswer($number){
    $query = "SELECT * FROM tbl_excel_ans WHERE quesNo ='$number'";
    $getData = $this->db->select($query);
    return $getData;
  }
}
